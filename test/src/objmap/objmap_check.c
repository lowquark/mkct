...
START_TEST(consistent_create_and_find) {
  ...
  for(int i = 0 ; i < N ; i ++) {
    ...
    obj_t * new_obj = imap_obj_create(&map, key);

    obj_t * found_obj = imap_obj_find(&map, key);
...
START_TEST(consistent_clear_and_find) {
  ...
  // create a slew of objects
  for(int i = 0 ; i < N ; i ++) {
    obj_t * new_obj = imap_obj_create(&map, i*507);

    obj_t * found_obj = imap_obj_find(&map, i*507);
...
START_TEST(consistent_destroy_and_find) {
  ...
  for(int i = 0 ; i < N ; i ++) {
    keys[i]    = rand();
    indices[i] = i;
    objs[i]    = imap_obj_create(&map, keys[i]);
  ...
  for(int i = 0 ; i < N / 2 ; i ++) {
    ...
    ck_assert_int_eq(imap_obj_destroy(&map, keys[idx]), 1);

    // try to find it, should fail
    ck_assert_ptr_null(imap_obj_find(&map, keys[idx]));
  }
  ...
  for(int i = N / 2 ; i < N ; i ++) {
    ...
    ck_assert_ptr_eq(imap_obj_find(&map, keys[idx]), objs[idx]);
  }
